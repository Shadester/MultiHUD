...
- **Widget overlay system** — weather, clock, meeting timer, and countdown widgets; each independently enabled and positionable at six locations (three top, three bottom); widgets sharing a position are grouped into one pill
- **Overlay safe-area profiles** — Full Frame, Meeting Safe, Top Strip, and Lower Third keep widgets clear of common video-app crops
...
- Virtual background via Robust Video Matting (Core ML) with Vision person segmentation (`VNGeneratePersonSegmentationRequest`) fallback
...
- Core ML Robust Video Matting (preferred) / Vision `VNGeneratePersonSegmentationRequest` fallback (person segmentation for virtual background)
...
- `settings.json` schema: `{ "cameraId": "", "blurBackground": false, "segQuality": "fast", "resolution": "720p", "opacity": 1.0, "overlaySafeArea": "fullFrame|meetingSafe|topStrip|lowerThird", "useRVM": true, "widgets": [{ "type": "weather|clock|countup|countdown", "position": "bottomLeft|bottomCenter|bottomRight|topLeft|topCenter|topRight", "enabled": bool, "startedAt": 0.0, "endsAt": 0.0 }] }`
...
- **Preview session retry**: `ContentView` uses `.task(id: ext.state.isActive)` with up to 12 retries (500 ms apart) to start the preview — the CMIOExtension device may not be in the discovery list immediately after the activation callback fires.
- **Single host instance**: `MultiHUDApp` holds an advisory lock in the shared app group; a duplicate host activates the existing app and exits. The lock is released just before the intentional fresh-install relaunch.
