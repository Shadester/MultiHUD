    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            TabView {
                CameraTabView(
                    ...
                    activelyUsingRVM: $activelyUsingRVM,
                    rvmFailureReason: $rvmFailureReason,
                    requestCameraAccess: requestCameraAccess,
                    stopPreview: stopPreview,
                    clearBackground: clearBackground,
                    importBackground: importBackground(from:),
                    readCamStatus: readCamStatus
                )
        ...
        .onChange(of: cameraStatus) { _, newStatus in
            ...
            if newStatus == .authorized, showPreview, ext.state.isActive, previewSession == nil {
                Task(priority: .userInitiated) { await startPreviewWithRetry() }
            }
        }
...
private struct CameraTabView: View {
    ...
    @Binding var hasBackground: Bool
    @Binding var showBackgroundPicker: Bool
    @Binding var backgroundError: String?
    @Binding var showEngineInfo: Bool
    @Binding var activelyUsingRVM: Bool?
    @Binding var rvmFailureReason: String?

    let requestCameraAccess: () async -> Void
    let stopPreview: () -> Void
    let clearBackground: () -> Void
    let importBackground: (URL) -> Void
    let readCamStatus: () -> Void
            ...
            Section("Access") {
                HStack(spacing: 8) {
                    ...
                    default:
                        ...
                        Spacer()
                        Button("Grant Access") { Task(priority: .userInitiated) { await requestCameraAccess() } }
                            .buttonStyle(.borderedProminent).controlSize(.small)
                    }
                }
...
private struct WidgetsTabView: View {
    @Environment(AppSettings.self) private var settings
    ...
    var body: some View {
        ...
        Form {
            ...
            Section("Weather") {
                WidgetRowView(label: "Weather", enabled: s.weatherEnabled,
                              position: s.weatherPosition, preview: "Temperature")
            }

            Section("Clock") {
                WidgetRowView(label: "Clock", enabled: s.clockEnabled,
                              position: s.clockPosition, preview: clockPreview)
            }

            Section("Meeting Timer") {
                WidgetRowView(label: "Meeting timer", enabled: s.countupEnabled,
            ...
            Section("Countdown") {
                WidgetRowView(label: "Countdown", enabled: s.countdownEnabled,
...
private struct PositionGridCell: Identifiable {
    let id: String
    let row: Int
    let col: Int
}
...
private struct PositionGridPicker: View {
    ...
    private let cells: [PositionGridCell] = [
        PositionGridCell(id: "topLeft",      row: 0, col: 0),
        PositionGridCell(id: "topCenter",    row: 0, col: 1),
        PositionGridCell(id: "topRight",     row: 0, col: 2),
        PositionGridCell(id: "bottomLeft",   row: 1, col: 0),
        PositionGridCell(id: "bottomCenter", row: 1, col: 1),
        PositionGridCell(id: "bottomRight",  row: 1, col: 2),
    ]
            ...
            ForEach(0..<2, id: \.self) { row in
                HStack(spacing: 3) {
                    ForEach(cells.filter { $0.row == row }) { cell in
                        PositionGridCellView(cell: cell, position: $position)
                    }
                }
            }
        }
...
private struct PositionGridCellView: View {
    let cell: PositionGridCell
    @Binding var position: String

    private var selected: Bool { position == cell.id }
